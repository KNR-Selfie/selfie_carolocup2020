...
Parking_manager::Parking_manager(const ros::NodeHandle &nh, const ros::NodeHandle &pnh):
                nh_(nh),
                pnh_(pnh),
                search_server_(nh_, "search",  false)
{
  search_server_.registerGoalCallback(boost::bind(&Parking_manager::init, this)); 
//  search_server_.registerPreemptCallback(boost::bind(&Parking_manager::preemptCB, this)); TODO
// TODO Result CB
...
bool Parking_manager::init()
{
  this->obstacles_sub = nh_.subscribe("/obstacles", 10, &Parking_manager::manager, this);
...
void Parking_manager::manager(const selfie_msgs::PolygonArray &msg)
{ 
...
void Parking_manager::filter_boxes(const selfie_msgs::PolygonArray &msg)
{
...
bool Parking_manager::find_free_places()///TODO naprawić kolejność wykrywania miejsca obecnie freeplace oddaje najdalsze miejsce a nie najbliższe
{
...
void Parking_manager::send_goal()
{
  /*
...
void Parking_manager::display_place(Box &place, const std::string &name)
{
...
void Parking_manager::display_places(std::vector<Box> &boxes, const std::string &name)
  {
    visualization_msgs::Marker marker;
